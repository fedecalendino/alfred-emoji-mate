...
USER_AGENT = (
    "Alfred-Workflow/" + __version__ + " (+http://www.deanishe.net/alfred-workflow)"
)
    ...
    def _get_encoding(self):
        ...
        if not self.stream:  # Try sniffing response content
            ...
            if self.mimetype == "text/html":  # sniff HTML headers
                m = re.search(
                    rb"""<meta.+charset=["']{0,1}(.+?)["'].*>""", self.content
                )
                if m:
...
def request(
    ...
    * ``mimetype`` is optional. If not provided, :mod:`mimetypes` will
    ...
    encodings = [
        s.strip() for s in headers.get("accept-encoding", "").split(",") if s.strip()
    ]
