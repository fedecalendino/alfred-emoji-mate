...
def _deprecation(message, stacklevel=3):
    message = (message + "\n") if message else ""
    warnings.warn(
        "%sTo hide this warning, pin/downgrade the package to 'emoji~=1.6.3'"
        % (message,),
        DeprecationWarning,
...
def _deprecation_removed(name, message=""):
    _deprecation(
        "'emoji.%s' is deprecated and will be removed in version 2.0.0. %s"
        % (name, message),
        4,
    )
...
def emojize(
    ...
    if use_aliases is _DeprecatedParameter:
        ...
        _deprecation(
            "The parameter 'use_aliases' in emoji.emojize() is deprecated and will be removed in version 2.0.0. Use language='alias' instead."
        )

    if use_aliases or language == "alias":
        if language not in ("en", "alias"):
            warnings.warn(
                "use_aliases=True is only supported for language='en'. "
    ...
    EMOJI_UNICODE = (
        unicode_codes.EMOJI_ALIAS_UNICODE_ENGLISH
        if use_aliases
        else unicode_codes.EMOJI_UNICODE[language]
    )
    pattern = re.compile(
        "(%s[\\w\\-&.’”“()!#*+?–,/]+%s)" % delimiters, flags=re.UNICODE
    )
    ...
    def replace(match):
        ...
        if version is not None:
            if (
                emj in unicode_codes.EMOJI_DATA
...
def demojize(
    ...
    if use_aliases is _DeprecatedParameter:
        ...
        _deprecation(
            "The parameter 'use_aliases' in emoji.demojize() is deprecated and will be removed in version 2.0.0. Use language='alias' instead."
        )
    ...
    if language == "alias":
        ...
        warnings.warn(
            "use_aliases=True is only supported for language='en'. It is recommended to use demojize(string, language='alias') instead",
            stacklevel=2,
        )
        language = "en"
            ...
            if "data" in sub_tree:
                ...
                if version is not None and emj_data["E"] > version:
                    ...
                    if use_aliases and "alias" in emj_data:
                        replace_str = (
                            delimiters[0] + emj_data["alias"][0][1:-1] + delimiters[1]
                        )
                    else:
                        replace_str = (
...
def replace_emoji(string, replace="", language=_DeprecatedParameter, version=-1):
    ...
    if language is not _DeprecatedParameter:
        _deprecation(
            "The parameter 'language' in emoji.replace_emoji() is deprecated and will be removed in version 2.0.0."
        )
...
def get_emoji_regexp(language=None):
    ...
    _deprecation_removed(
        "get_emoji_regexp()",
        "If you want to remove emoji from a string, consider the method emoji.replace_emoji(str, replace='').",
    )
...
def distinct_emoji_lis(string, language=_DeprecatedParameter):
    ...
    _deprecation_removed(
        "distinct_emoji_lis()", "Use method emoji.distinct_emoji_list(str) instead."
    )

    distinct_list = list({e["emoji"] for e in emoji_lis(string)})
